...
const Navbar = () => {
  ...
  const getUser = async () => {
    try {
      const userInfo = await axios.get(
        "https://thang-todo.herokuapp.com/api/users/me"
      );
  ...
  const handleLogout = async () => {
    try {
      await axios.post("https://thang-todo.herokuapp.com/api/authen/logout");
      navigate("/auth");
    } catch (err) {
  ...
  return (
    <div className="nav-container grid" ref={navRef}>
      ...
      <div
        className={`${showNav ? "nav-userinfo" : "nav-userinfo-responsive"}`}
      >
        ...
        <button className="nac-btn-close" onClick={() => setShowNav(true)}>
          <FaTimes />
        </button>
      </div>
